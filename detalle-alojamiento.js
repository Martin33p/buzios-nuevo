...
async function cargarAlojamientoIndividual() {
  ...
  for (let i = 1; i <= 10; i++) {
  const url = alojamiento[`Imagen${i}_URL`];  // Clave con template literal corregida
  if (url) {
    const img = document.createElement("img");
    img.src = url;
    img.alt = `${alojamiento["Nombre"]} - Imagen ${i}`;  // Template literal corregido para 'alt'
  ...
  if (id) {
    ...
    // Usamos comillas invertidas para la URL con el template literal
...
`;
  ...
  document.getElementById("volver-btn").innerHTML = 
    <a href="/alojamientos.html" class="volver-btn">← Volver a alojamientos</a>
  ;
}
